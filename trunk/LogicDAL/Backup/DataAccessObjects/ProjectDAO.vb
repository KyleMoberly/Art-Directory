...
Public Class ProjectDAO
    ...
    Public Shared Function ListAllProjects() As List(Of IProject)
        ...
        Try
            While dr.Read()
                ...
                aProject.Name = dr("projectName")
                aProject.Dates = dr("projectDates")
                aProject.URL = dr("projectURL")
                aProject.Description = dr("projectDesc")
    ...
    Public Shared Function GetProjectByID(ByVal id As Integer) As IProject
        ...
        Try
            Logger.Write("Starting to execute: " & sql)
            dr.Read()
            aProject = New State(dr("projectID"))
            aProject.Name = dr("projectName")
            aProject.Dates = dr("projectDates")
            aProject.URL = dr("projectUrl")
            aProject.Description = dr("projectDesc")
    ...
    Public Shared Sub CreateProject(ByVal newProject As IProject)
        Dim db As Database = DatabaseFactory.CreateDatabase(ConstantsLibrary.LOCAL_ART_DB)
        Dim sql As String = "INSERT INTO Projects (projectName, projectDates, projectURL, projectDesc, listId)_ VALUES ('" _
                            & newProject.Name & "','" & newProject.Dates & "','" & newProject.URL & "','" & newProject.Description & "','" & newProject.ArtListing.PrimaryKey & "')"
        Dim cmd As DbCommand = db.GetSqlStringCommand(sql)

        Try
            Logger.Write("Starting to Execute: " & sql)
            Dim rowsAffected As Integer = db.ExecuteNonQuery(cmd)
            Logger.Write(rowsAffected & " project was inserted with values: " & newProject.PrimaryKey & " " & newProject.Name & " " & newProject.Dates & " " & newProject.URL & " " & newProject.Description & " " & newProject.ArtListing.PrimaryKey)
        Catch ex As Exception
    ...
    Public Shared Sub UpdateProject(ByVal aProject As IProject)
        ...
        Dim sql As String = "UPDATE Projects SET " _
                            ...
                            & "', listId = '" & aProject.ArtListing.PrimaryKey _
